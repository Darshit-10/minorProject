...
def test_synthetic_data():
    ...
    def plot_boundaries(w1, w2, p1, p2, acc1, acc2, fname): 
        ...
        plt.plot([x1, x2], [y1, y2], 'c-', linewidth=3, label=f"Acc={acc1:.2f}; p% rule={p1:.0f}% - Original")
        y1, y2 = ut.get_line_coordinates(w2, x1, x2)
        plt.plot([x1, x2], [y1, y2], 'b--', linewidth=3, label=f"Acc={acc2:.2f}; p% rule={p2:.0f}% - Constrained")

        plt.tick_params(axis='x', which='both', bottom='off', top='off', labelbottom='off')  # don't need the ticks to see the data distribution
