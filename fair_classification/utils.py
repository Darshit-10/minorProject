...
def train_model(x, y, x_control, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, gamma=None):
    ...
    return w.x
def train_test_single_fold(train_data, test_data, fold_num, output_folds,loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh,gamma):
...
def compute_cross_validation_error(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh_arr,gamma):
    ...
    output_folds = Queue()
    processes = [Process(target=train_test_single_fold, args=(train_folds[x], test_folds[x], x, output_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh_arr[x],gamma)) for x in range(num_folds)]
    ...
    results = sorted(results, key=lambda x: x[0]) # sort with respect to fold num
    for res in results:
...
def print_covariance_sensitive_attrs(model, x_arr, y_arr_dist_boundary, x_control, sensitive_attrs):
    ...
    for attr in sensitive_attrs:
        ...
        attr_arr = [int(i) for i in attr_arr]
...
def get_constraint_list_cov(x_train, y_train, x_control_train, sensitive_attrs, sensitive_attrs_to_cov_thresh):
    ...
    for attr in sensitive_attrs:
        # Check if the attribute exists in the threshold dictionary
        if attr not in sensitive_attrs_to_cov_thresh:
            # Add a default threshold value
            sensitive_attrs_to_cov_thresh[attr] = 0.0  # You can set any default threshold value here

        attr_thresh = sensitive_attrs_to_cov_thresh[attr]
        ...
        if index_dict is None:  # binary attribute
            c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args': (x_train, y_train, attr_arr_transformed, attr_thresh, False)})
            constraints.append(c)
        else:  # categorical attribute
            for attr_val, ind in index_dict.items():
                attr_name = attr_val
                thresh = attr_thresh.get(attr_name, 0.0)  # Get the threshold for this attribute value
                t = attr_arr_transformed[:, ind]
