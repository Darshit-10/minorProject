...
from cvxpy import multiply as mul_elemwise

import dccp
from dccp.problem import dccp
import utils as ut
...
def get_clf_stats(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs):
    ...
    s_attr = next(iter(sensitive_attrs)) # for now, lets compute the accuracy for just one sensitive attr
...
def get_fpr_fnr_sensitive_features(y_true, y_pred, x_control, sensitive_attrs, verbose = False):
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            acc = float(np.sum(np.array(y_true_local==y_pred_local))) / len(y_true_local)
